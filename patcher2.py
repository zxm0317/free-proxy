                    ...
                    for call in step['tool_calls']:
                        if call['name'] in ['replace_file_content', 'multi_replace_file_content']:
                            ...
                            if call['name'] == 'replace_file_content':
                                chunks = [{
                                    'TargetContent': args.get('TargetContent', '').replace('\r\n', '\n'),
                                    'ReplacementContent': args.get('ReplacementContent', '').replace('\r\n', '\n')
                                }]
                            else:
                                try:
                                    ...
                                    for c in parsed:
                                        chunks.append({
                                            'TargetContent': c.get('TargetContent', '').replace('\r\n', '\n'),
                                            'ReplacementContent': c.get('ReplacementContent', '').replace('\r\n', '\n')
                                        })
                                except:
                                    pass
                            ...
                            for chunk in chunks:
                                ...
                                if tc:
                                    if content.count(tc) == 1:
                                        ...
                                        print("Could not find TargetContent! Length:", len(tc), "Sample:", repr(tc[:20]))
