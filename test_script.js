        ...
        async function loadStatuses() {
            ...
            allProviderStatuses = state.statuses;
            renderProviderCards();
            populateAddProviderSelect();
            renderProviderTabs();
            refreshChatOptions();
        ...
        window.toggleModelEnabled = async function(providerName, checked) {
            try {
                const res = await fetch(`/api/providers/${providerName}/toggle`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                return `
                    <tr style="border-bottom: 1px solid var(--line); opacity: ${rowOpacity}; transition: opacity 0.2s;" 
                        ...
                        <td style="padding: 12px; vertical-align: middle; text-align: right;">
                            <label class="switch">
                                <input type="checkbox" ${item.enabled ? 'checked' : ''} onchange="toggleModelEnabled('${item.provider}', this.checked)">
